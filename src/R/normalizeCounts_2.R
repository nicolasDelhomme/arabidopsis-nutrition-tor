...
#' ```{r set up, echo=FALSE}
#' knitr::opts_knit$set(root.dir="/mnt/picea/projects/arabidopsis/jhanson/arabidopsis-nutrition-TOR/analysis")
#' ```
#' Libraries
...
#samples <- subset(samples, samples[,1] %in% colnames(kg))
...
#'## Use of a PCA
#'### Colour coded by Nutrition and AZD treatment
...
plot(pc$x[,1],
     ...
     pch=c(17,19)[factor(substr(sub(".*_","",samples$SciLifeID),1,1))])
...
#text(pc$x[,1],pc$x[,2],cex=.7,sub(".*_","",samples$SciLifeID),adj = c(1,1))
